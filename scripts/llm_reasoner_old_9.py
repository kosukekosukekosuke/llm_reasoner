...
"""
LLM Reasoner ノード（Phase2：abstain(-1) ＋ OOD ゲート追加版）。

- Phase1（cos 分布＋相対 cos）の機能に加えて、
  「危ないときは無理にノードを選ばず棄権する」安全設計を導入した版。
- 通常は confidence と cosine（相対値）から combined_score を計算し、
  閾値を満たせばそのノードを採択する。

Phase2 で追加したポイント：
- OOD ゲート：
  - max_cos が低い、あるいは top1−top2 の margin が小さいときに
    「どのノードとも十分に似ていない」と判断するためのしきい値を追加。
- conf / cosine / combined のすべてがしきい値を満たさない場合は accept せず再推論。
- すべての試行で受理条件を満たせなかった場合は、
  node_id = -1（abstain ノード）として棄権し、
  is_abstain = true, accept_reason = "abstain_no_accept" を設定。
- abstain のときも、最後の試行の confidence / cosine_raw / cosine / combined_score を
...
class LLMReasonerNode:
    ...
    def callback_query(self, msg):
        ...
        #   - OOD 由来の abstain
        #   - しきい値未満による abstain_no_accept
        # =====================================================
        if decision_source is not None and decision_source.startswith("abstain_"):
            # ---------------------------------------------
            # 1) OOD ゲート由来の abstain (abstain_ood_*)
            #    → LLM は呼んでいないので confidence / combined は None のまま。
            #    → 埋め込み側の「max cosine」を top-level に写しておく。
            # ---------------------------------------------
            ...
            # ---------------------------------------------
            # 2) 再推論まで回したが閾値を満たす試行が 1つも無かった abstain
            #    (abstain_no_accept)
            ...
            # ---------------------------------------------
                ...
                for a in attempts_log:
                    ...
                    # combined_score が None の試行は比較対象外にしておく
                    if score is None:
                        continue
